...
function count_by_claim($table){
  global $db;
  if(tableExists($table))
  {
    $sql    = "SELECT COUNT(claim_id) AS total FROM ".$db->escape($table);
    $result = $db->query($sql);
     return($db->fetch_assoc($result));
  }
}
...
function count_by_reimbursements($table){
  global $db;
  if(tableExists($table))
  {
    $sql    = "SELECT COUNT(reimbursement_id) AS total FROM ".$db->escape($table);
    $result = $db->query($sql);
     return($db->fetch_assoc($result));
  }
}
