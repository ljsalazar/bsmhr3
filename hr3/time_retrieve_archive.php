<?php
	$page_title = 'Time Retrieve Prompt';
				...
				<div class="col-md-12">
					<div class="panel">
						<div class="jumbotron text-center">
							<?php
								if(isset($_POST['yes'])) {
									$query1 = $conn->query("INSERT INTO time_attendance SELECT * FROM time_attendance_archive WHERE time_id=$time_id");
									$query = $conn->query("DELETE FROM time_attendance_archive WHERE time_id=$time_id");
									$session->msg('s',"Successfully Retrieved");
									
									#header("Location:timesheet_index.php");
									echo "<script>window.location.href='time_archive.php';</script>";
								}
							?>
							<h2>Are you sure you want to retrieve?</h2>	
							<form method="post" action="">
								</br>
								<button type="submit" name="yes" class="btn btn-primary" value="yes">Yes</button>
							</form></br>
							<button name="cancel" class="btn" onclick="location.href='time_archive.php'">Cancel</button>
						</div>
					</div>
				</div>
