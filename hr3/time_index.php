		...
		<div class="panel">
			<div class="jumbotron text-center">
				...
				<?php 
					...
					$q_student = $conn->query("SELECT * FROM `tblschedule` WHERE empid = $user_id AND status = 1") or die(mysqli_error());
					#$fetch = mysqli_fetch_array($q_student);
					while($fetch = mysqli_fetch_array($q_student)) {
						if (isset($fetch['days'])){
							$days = $fetch['days'];	
							if ($dayOfWeek == "Monday"){
								if (strpos($days, 'Mon') !== false){
									echo "<form action='time_index_loggedin.php' method='POST'>
									<input type='submit' name='login' value='Start your shift for the day' class='btn btn-primary' style='font-size:20px'>
									</form>";
									} else {
									#echo "<p style='font-size:20px'>No shift for the day</p>";
								}
							}
							...
							echo "<p style='font-size:20px'>Shift not set or granted</p>";
						}
					}
				?>
