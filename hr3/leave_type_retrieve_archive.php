<?php
	$page_title = 'Leave Type Retrieve Prompt';
	...
	$leavetype_id = $_GET['leavetype_id'];
					...
					<div class="panel">
						...
						$l_types = $_POST['leavetype'];
     					$l_description  = $_POST['description'];
									...
									echo "<script>window.location.href='leave_type_archive.php';</script>";
								}
							?>
							...
							<?php
							$query2 = $conn->query("SELECT * FROM tblleavetype_archive WHERE id=$leavetype_id");
						...
						<?php }?>
							...
							<button name="cancel" class="btn" onclick="location.href='leave_type_archive.php'">Cancel</button>
						</div>
					</div>
				</div>
