<?php
	$page_title = 'Claim Accept Prompt';
								...
								$read = 1;
									...
									while($user_data = mysqli_fetch_array($query1)){
										...
										if ($getusername == $username){
											$session->msg('d',"You can't accept your own claims");
											echo "<script>window.location.href='claim_history.php';</script>";
											} else {
											$conn->query("UPDATE claim SET status='Accepted by $name' WHERE claim_id='$claim_id'") or die(mysqli_error());
											$conn->query("UPDATE claim SET accepted=1 WHERE claim_id='$claim_id'") or die(mysqli_error());
											$conn->query("UPDATE users SET claim_notif=claim_notif+$read WHERE id=$id");
											$session->msg('s',"Claim Successfully Accepted");
										}
									}
									echo "<script>window.location.href='claim_history.php';</script>";
								}
